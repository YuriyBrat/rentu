...
export default function RentOptionsSection({
   ...
   const isRented = statusRent === 'rentRented';
   ...
   return (
      ...
      >
         ...
         <Grid container spacing={1.5}>
            <Grid item xs={12}>
               <TextField
                  label="Назва для оренди"
                  value={rent.rentTitle}
                  onChange={(e) => setRent('rentTitle', e.target.value)}
                  fullWidth
                  sx={fieldSx}
                  placeholder="Окремий заголовок саме для оренди..."
               />
            </Grid>
               ...
               <>
                  ...
                  <Grid item xs={12} md={4}>
                     <TextField
                        type="date"
                        label="Дата здачі"
                        value={rent.rentStory?.rentedAt || ''}
                        onChange={(e) => setRentStory('rentedAt', e.target.value)}
                        fullWidth
                        sx={fieldSx}
                        InputLabelProps={{ shrink: true }}
                     />
                  ...
                  <Grid item xs={12} md={4}>
                     <TextField
                        select
                        label="Ким здано"
                        value={rent.rentStory?.rentedBy || ''}
                        onChange={(e) => setRentStory('rentedBy', e.target.value)}
                        fullWidth
                        sx={fieldSx}
                        SelectProps={{ MenuProps: selectMenuProps }}
                     >
                        {RENTED_BY_OPTIONS.map((x) => (
                           <MenuItem key={x.value} value={x.value}>
                              {x.label}
                           </MenuItem>
                        ))}
                  ...
                  <Grid item xs={12} md={4}>
                     <TextField
                        label="Нотатка по здачі"
                        value={rent.rentStory?.note || ''}
                        onChange={(e) => setRentStory('note', e.target.value)}
                        fullWidth
                        sx={fieldSx}
                     />
                  </Grid>
