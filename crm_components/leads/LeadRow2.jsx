   ...
   return (
      ...
      >
         ...
         <Stack spacing={0.1} minWidth={0}>
            <Typography sx={{ color: 'rgba(255,255,255,0.54)', fontSize: 11 }}>
               {label}
            </Typography>
            <Typography sx={{ color: '#fff', fontSize: 13, fontWeight: 800 }} noWrap>
               {value || '—'}
            </Typography>
         </Stack>
      </Stack>
         ...
         <Box sx={{ p: 1.2 }}>
            ...
            >
               <Stack spacing={0.7} minWidth={0}>
                  ...
                  <Typography sx={{ color: '#fff', fontWeight: 900, fontSize: 17 }} noWrap>
                     {item.name}
                  </Typography>

                  <Typography sx={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }} noWrap>
                     {(item.phones || []).join(', ') || 'Без телефону'}
                  </Typography>

                  {!!item.emails?.length && (
                     <Typography sx={{ color: 'rgba(255,255,255,0.5)', fontSize: 12 }} noWrap>
                        {item.emails.join(', ')}
                     </Typography>
                  )}
               </Stack>
               ...
               <Stack spacing={0.9} minWidth={0}>
                  ...
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.8 }}>
                     <MetaPill
                        icon={<SellRoundedIcon sx={{ fontSize: 16 }} />}
                        label="Бюджет max"
                        value={item.budgetMax ? `${item.budgetMax.toLocaleString('uk-UA')} $` : '—'}
                     />
                     <MetaPill
                        icon={<InfoOutlinedIcon sx={{ fontSize: 16 }} />}
                        label="Актуальність"
                        value={item.actualityStatus}
                     />
                     <MetaPill
                        icon={<EventRoundedIcon sx={{ fontSize: 16 }} />}
                        label="Остання актуальність"
                        value={item.lastActualizedAt}
                     />
                  </Box>
               </Stack>

               <Stack spacing={0.8} alignItems={{ xs: 'stretch', lg: 'flex-end' }}>
                  <MetaPill
            ...
            {!!lastNote && (
               <Box sx={{ mt: 1 }}>
                  <Chip
                     label={`Остання нотатка: ${lastNote.text}`}
                     sx={{
                        maxWidth: '100%',
                        color: noteStyles[lastNote.type]?.color || '#fff',
                        bgcolor: noteStyles[lastNote.type]?.bg || 'rgba(255,255,255,0.08)',
                        border: noteStyles[lastNote.type]?.border || '1px solid rgba(255,255,255,0.08)',
                        '& .MuiChip-label': {
                           display: 'block',
                           whiteSpace: 'nowrap',
                           overflow: 'hidden',
